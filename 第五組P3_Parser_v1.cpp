/*
 * (P3) Recursive Descent Parser_v1 (第五組) 2017.06.28
 *		- 本程式以P4程式為基底撰寫改版，部分函式有所更動
 * Function:
 *		- Parsing：根據消除left recursion後的G1文法建構判斷式，分析輸入的statement是否合法
 *	　	- next_Token()：呼叫P4的Lexical Analyzer提取下一個token
 *		- match()：將當前token比對是否正確，並呼叫next_Token()提取下個token
 *		- 若遇到不合法情況，會結束Parsing，並印出當前執行階段
 *		- [同於P4程式]提取token時會做初步的syntax error回報(以Regular expression判定格式是否符合)
 * Input:
...
//參考課程PPT(02)page.68-70及中文課本Ch2.4.2
int match(string t)
{
	if(lookahead==t)
	{
...
int main()
{
	while(1)
	{
		...
		expr();					//開始執行Parsing
		if(lookahead=="EOF.(1)")	//表示正常結束（整句statement讀完）
		{
